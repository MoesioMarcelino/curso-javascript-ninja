(function(){
    ...
    */
    ...
    console.log(text.replace(/Manuel Marques de Sousa/, "Moesio Ferreira Marcelino"));
    ...
    */
    ...
    var regexDate = text.match(/(junho)|(julho)/g, function(capturaTotal){
        return capturaTotal;
    });
    ...
    */
    ...
    function replaceDate(){
        var dias = text.match(/ \d\d /g, function(capturaTotal){
            return capturaTotal;
        });

        dias = dias.map(function(value){
            return value.replace(value, value.substring(1,3));
        });

        var meses = regexDate.map(function(value){
            var mes = getMonthNumber(value);
            return mes;    
        });

        var anos = text.match(/\d\d\d\d|\d\d\d\d/g, function(capturaTotal){
            return capturaTotal;
        });

        var data1 = dias[0]+"/"+meses[0]+"/"+anos[0];
        var data2 = dias[1]+"/"+meses[1]+"/"+anos[1];
        
        text = text.replace(/13 de junho de 1804/, data1);
        text = text.replace(/18 de julho de 1875/, data2);
